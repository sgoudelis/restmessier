...
def get_nearby_objects_by_mid(mid, degrees):
    ...
    # find objects near
    nearby_objects = []
    nearby_objects_cursor = messier_collection.find( { 'loc' :
                              { '$geoWithin' :
                                { '$centerSphere' :
                                 [ [ center_object['loc']['coordinates'][0], center_object['loc']['coordinates'][1] ] , radians ] }
                               } 
                              }
                            )
...
def get_nearby_objects_by_coord(ra, dec, degrees):
    """
    Based on a given coordinate and angular distance, find nearby objects 
    """
